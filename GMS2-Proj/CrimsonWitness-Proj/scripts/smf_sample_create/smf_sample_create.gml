/// @description smf_sample_create(animationIndex, type, time)
...
www.TheSnidr.com
*/
var time, type, animationIndex, loop, quadratic, linear, bindPose, frameGrid, worldDQ, Qa, Qb, Qc, deltaWorldDQ, boneNum, frameNum, pDQ, cDQ, dWDQ, lr, ld, a, b, i, j, l, p, f, ma, mb, d, Q, bindDQ, localFrame, framePriority, returnSample;
...
bindPose = SMF_bindList[| animationIndex];
frameGrid = SMF_frameList[| animationIndex];
...
if quadratic
{
	...
	for (i = 0; i < boneNum; i ++)
	{
	    ...
	    for (j = 0; j < 8; j ++)
	    {
			...
			Q[j] = sqr(1 - d) * (Qa[j] + Qb[j]) / 2 + 2 * d * (1 - d) * Qb[j] + sqr(d) * (Qc[j] + Qb[j]) / 2;
	    } 
		bindDQ = bindPose[| i];
		...
		//Generate sample
	    for (var j = 0; j < 8; j ++){returnSample[i * 8 + j] = dWDQ[j];}
		cDQ = -1;
	}
}
if linear
